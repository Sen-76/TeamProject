    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (service.equals("ListAllFunction")) {
                ...
                if (submit == null) {
                    List<Function1> list = dao.viewFunctionListTranier(Loged.getUser_id(), 1, index);
                    List<Function1> TeamList = dao.viewTeamTrainer(Loged.getUser_id(), 1);
                    ...
                    if (team.equals("all") && search.equals("") && status.equals("all")) {
                        List<Function1> list = dao.viewFunctionListTranier(Loged.getUser_id(), 1, index);
                        List<Function1> TeamList = dao.viewTeamTrainer(Loged.getUser_id(), 1);
                        request.setAttribute("FunctionListTrainer", list);
                        request.setAttribute("TeamList", TeamList);
                    ...
                    if (search.equals("") && !team.equals("all")) {
                        List<Function1> searchByTeam = dao.searchByTeamTrainer(team, Loged.getUser_id(), 1);
                        List<Function1> TeamList = dao.viewTeamTrainer(Loged.getUser_id(), 1);
                    ...
                    if (!search.equals("") && team.equals("all")) {
                        List<Function1> searchByName = dao.searchByNameTrainer(Loged.getUser_id(), 1, search);
                    ...
                    if (search.equals("") && !status.equals("all")) {
                        List<Function1> searchByStatusTrainer = dao.searchByStatusTrainer(Loged.getUser_id(), 1, status);
                        List<Function1> TeamList = dao.viewTeamTrainer(Loged.getUser_id(), 1);
